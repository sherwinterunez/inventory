		      ...
		      if(preg_match('/\<ErrorCode\>(\d+)\<\/ErrorCode\>.+?\<Description\>(.+?)\<\/Description\>/si',$img,$mt)) {
		        ...
		        //$filenameonly = $documentId[1] . '.tiff';
			        ...
			        $partial = false;
							...
							// Network Error
							// HTML
							if(preg_match('/\<HTML\>/si',$img,$mt)) {
							} else
							if(preg_match('/Network\s+Error/si',$img,$mt)) {
							} else
							if(preg_match('/Service\s+Unavailable/si',$img,$mt)) {
							} else
							if(preg_match('/Script\s+timed\s+out/si',$img,$mt)) {
							} else
