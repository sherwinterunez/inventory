...
/*if(!($result = $appdb->query("select *,(extract(epoch from now()) - extract(epoch from pacsimage_createstamp)) as elapsedtime,extract(epoch from pacsimage_createstamp)::integer as unixstamp from tbl_pacsimage where pacsimage_status=2 and pacsimage_createstampunix=0 order by pacsimage_id asc limit $limit"))) {
...
while(1) {
	...
	if(!($result = $appdb->query("select *,(extract(epoch from now()) - extract(epoch from pacsimage_createstamp)) as elapsedtime,extract(epoch from pacsimage_createstamp)::integer as unixstamp from tbl_pacsimage where pacsimage_createstampunix=0 order by pacsimage_id asc limit $limit"))) {
		//echo "\n0 message. processOutbox done.\n";
		return false;
	}
	...
	if(!empty($result['rows'][0]['pacsimage_id'])) {
		...
		foreach($result['rows'] as $k=>$v) {
			...
			if(!($result = $appdb->update("tbl_pacsimage",array('pacsimage_createstampunix'=>$v['unixstamp'],'pacsimage_updatestamp'=>'now()'),"pacsimage_id=".$v['pacsimage_id']))) {
				return false;
			}
...
// select *,(extract(epoch from now()) - pacsimage_createstampunix)::integer as elapsedtime from tbl_pacsimage where (extract(epoch from now())-pacsimage_createstampunix)> 10 and pacsimage_createstampunix>0;
